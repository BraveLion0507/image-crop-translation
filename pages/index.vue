...
const mode = useSessionStorage("mode", store.mode);

function handleModeChange(value: string) {
  // Handle the changed value here
  store.changeMode(value); // Call the action in the store to update the mode state
...
const handleImageCrop = ({ coordinates, cropFile }) => {
  store.changeCropImage(cropFile);
  store.changeVisibleArea(coordinates);
};
...
async function upload() {
  ...
  console.log("store.cropImage.value", store.cropImage);
  ...
  formData.append(
    "file",
    store.mode == "default" ? file.value : store.cropImage
  );
  ...
  await new Promise((resolve) => {
    img2.onload = () => {
      if (store.mode == "crop")
        canvasCtx.drawImage(
          img2,
          store.visibleArea.left,
          store.visibleArea.top
        );
      else {
...
watch(resultBlob, (blob) => {
  ...
  console.log("resultUri.value", resultUri.value);
});
...
function translateAgain() {
  console.log("translate again");
  fileUri.value = resultUri.value;
  resultUri.value = "";
  // file.value = null;
  taskId.value = "";
  errorId.value = "";
      ...
      >
        ...
        >
          <div class="flex flex-col gap-2">
            ...
            <img
              v-if="store.mode == 'default'"
              :src="fileUri"
              class="object-contain max-w-100vw sm:max-w-[calc(100vw-20rem)] sm:h-[calc(100vh-12rem)]"
            />
            <BaseCropper
              v-if="store.mode == 'crop'"
              :image-source="fileUri"
              class="object-contain max-w-100vw sm:max-w-[calc(100vw-20rem)] sm:h-[calc(100vh-12rem)]"
              @on-crop="handleImageCrop"
            />
          </div>
          ...
          <div class="flex flex-col gap-2 pointer-events-none">
            ...
            <label class="pointer-events-auto">
              <span class="ml-2 text-sm">Mode</span>
              <UListbox
                v-model="mode"
                class="w-56"
                :items="modes"
